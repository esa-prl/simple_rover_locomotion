...
SimpleRoverLocomotion::SimpleRoverLocomotion(rclcpp::NodeOptions options,
  ...
  steering_in_progress_(false)
{
  ...
  if (!check_steering_limitations()) {
    RCLCPP_ERROR(
      this->get_logger(),
      "%s not able to work, since the robot model has an invalid steering configuration.",
      node_name.c_str());
  }
}
...
bool SimpleRoverLocomotion::check_steering_limitations()
{
  ...
  // Checks how many legs are non steerable and sets the centre of rotation
  // accortingly
  if (non_steerable_legs.size() == 0) {
    ...
    centre_of_rotation_x =
      non_steerable_legs[0]->steering_motor->global_pose.position.x;

    // Can be any value. 0 makes the most sense.
    centre_of_rotation_y = 0;
  } else if (non_steerable_legs.size() == 2) {
    RCLCPP_WARN(this->get_logger(),
                "NUMBER OF NON STEERABLE LEGS: %u",
                non_steerable_legs.size());

    // In case there are multiple fixed wheels their axes must align
    // TODO: Check in which direction the wheels point in their 0 position and
    // apply that to the input limitation etc.
    ...
    centre_of_rotation_x =
      (non_steerable_legs[0]->steering_motor->global_pose.position.x +
       non_steerable_legs[1]->steering_motor->global_pose.
       position.x) / 2;

    // This should be 0 in most cases
    centre_of_rotation_y =
      (non_steerable_legs[0]->steering_motor->global_pose.position.y +
       non_steerable_legs[1]->steering_motor->global_pose.
       position.y) / 2;
  } else {
...
//   RCLCPP_INFO(this->get_logger(), "ENABLING SimpleRoverLocomotion using a
// custom transition");
  ...
  const geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  // TODO: Check if Joint State is older than a certain age and if send a
  // warning/refuse execution accordingly
  ...
  for (std::shared_ptr<LocomotionMode::Leg>leg : legs_) {
    ...
    double l;           // [m] Radial Distance from Wheel Steering center to
                        // Origin
    double beta_offset; // [rad] Static offset to calculate steering angle from
                        // computed angle
    ...
    double beta_current = leg->steering_motor->joint_state.position[0]; // Current
                                                                        // steering
                                                                        // Angle

    // RCLCPP_INFO(this->get_logger(), "Current Steering for %s: %f [rad]",
    // leg->steering_motor->joint->name.c_str(), beta_current);
    ...
    if (stop_rover) {
      ...
      l     =
        sqrt(pow(offset_str_position_x, 2.0) + pow(offset_str_position_x, 2.0));

      // TODO: Could implment this into main LocomotionMode class and add a
      // field wheel diameter. Also make sure the wheel link is a cylinder.
      if (leg->driving_motor->link->collision->geometry->type ==
          urdf::Geometry::CYLINDER) {
        std::shared_ptr<urdf::Cylinder> cyl =
          std::static_pointer_cast<urdf::Cylinder>(
        ...
        RCLCPP_WARN(
          this->get_logger(),
          "Wheel Link: %s collision geometry should be a cylinder! Wheel radius set to %f [m]",
          leg->driving_motor->link->name,
          r);
      }
      ...
      if (((beta_steer < upper_position_limit) &&
           (beta_steer > lower_position_limit + M_PI)) ||
          ((beta_steer > lower_position_limit) &&
           (beta_steer < upper_position_limit - M_PI)))
      {
        // Sets steering angle so it's the closest to the current steering angle
        double beta_1 = beta_steer;                              // Option one
                                                                 // is the
                                                                 // computed
                                                                 // steering
                                                                 // angle
        double beta_2 = beta_steer - copysign(M_PI, beta_steer); // Option two
                                                                 // is the
                                                                 // computed
                                                                 // steering
                                                                 // angle
      ...
      // Only computes the driving velocity if the wheel is close enough to the
      // target position
      if (steering_in_progress_ ||
          (abs(beta_steer - beta_current) > steering_margin_)) {
        steering_in_progress_ = true;
        phi_dot               = 0.0;
      } else {
        // Computes Driving Speed
        // TODO: Compute the wheel speeds from the current wheel orientations
        // and not the set wheel orientations.
        phi_dot =
          (sin(alpha + beta) * x_dot - cos(alpha + beta) * y_dot - l * cos(beta) *
           theta_dot) / r;
  ...
  // The driving command must be set to zero if ANY steering position has not
  // been reached yet.
  if (steering_in_progress_) {
    for (unsigned i = 0; i < driving_command_array_msg.joint_command_array.size();
         i++) {
  ...
  // Gazebo does not like to receive the steering and driving commands in two
  // different messages.
  // Publish Message
